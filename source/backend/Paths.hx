...
class Paths {
	...
	inline static public function txt(key:String, ?folder:String) return getPath('data/$key.txt', TEXT, folder, true);
	inline static public function xml(key:String, ?folder:String) return getPath('data/$key.xml', TEXT, folder, true);
	inline static public function json(key:String, ?folder:String) return getPath('data/$key.json', TEXT, folder, true);
	inline static public function shaderFragment(key:String, ?folder:String) return getPath('shaders/$key.frag', TEXT, folder, true);
	inline static public function shaderVertex(key:String, ?folder:String) return getPath('shaders/$key.vert', TEXT, folder, true);
	inline static public function lua(key:String, ?folder:String) return getPath('$key.lua', TEXT, folder, true);
	...
	inline static public function sound(key:String, ?modsAllowed:Bool = true):Sound return returnSound('sounds/$key', modsAllowed);
	inline static public function music(key:String, ?modsAllowed:Bool = true):Sound return returnSound('music/$key', modsAllowed);
	inline static public function inst(song:String, ?modsAllowed:Bool = true):Sound return returnSound('${formatToSongPath(song)}/Inst', 'songs', modsAllowed);
	...
	inline static public function soundRandom(key:String, min:Int, max:Int, ?modsAllowed:Bool = true) return sound(key + FlxG.random.int(min, max), modsAllowed);
	...
	static public function getAtlas(key:String, ?parentFolder:String = null, ?allowGPU:Bool = true):FlxAtlasFrames {
		...
		if (OpenFlAssets.exists(myXml) #if MODS_ALLOWED || (#if sys FileSystem.exists(myXml) #end && (useMod = true)) #end) {
			#if MODS_ALLOWED
			return FlxAtlasFrames.fromSparrow(imageLoaded, (useMod ? #if sys File.getContent(myXml) #else myXml #end : myXml));
			#else
			return FlxAtlasFrames.fromSparrow(imageLoaded, myXml);
			#end
		} else {
			var myJson:Dynamic = getPath('images/$key.json', TEXT, parentFolder, true);
			if (OpenFlAssets.exists(myJson) #if MODS_ALLOWED || (#if sys FileSystem.exists(myJson) #end && (useMod = true)) #end) {
				#if MODS_ALLOWED
				return FlxAtlasFrames.fromTexturePackerJson(imageLoaded, (useMod ? #if sys File.getContent(myJson) #else myJson #end : myJson));
				#else
	...
	inline static public function getSparrowAtlas(key:String, ?parentFolder:String = null, ?allowGPU:Bool = true):FlxAtlasFrames {
		...
		#end
		return FlxAtlasFrames.fromSparrow(imageLoaded, (xmlExists ? #if sys File.getContent(xml) #else xml #end : getPath(Language.getFileTranslation('images/$key') + '.xml', TEXT, parentFolder)));
		#else
	...
	inline static public function getPackerAtlas(key:String, ?parentFolder:String = null, ?allowGPU:Bool = true):FlxAtlasFrames {
		...
		#end
		return FlxAtlasFrames.fromSpriteSheetPacker(imageLoaded, (txtExists ? #if sys File.getContent(txt) #else txt #end : getPath(Language.getFileTranslation('images/$key') + '.txt', TEXT, parentFolder)));
		#else
	...
	inline static public function getAsepriteAtlas(key:String, ?parentFolder:String = null, ?allowGPU:Bool = true):FlxAtlasFrames {
		...
		#end
		return FlxAtlasFrames.fromTexturePackerJson(imageLoaded, (jsonExists ? #if sys File.getContent(json) #else json #end : getPath(Language.getFileTranslation('images/$key') + '.json', TEXT, parentFolder)));
		#else
	...
	#if flxanimate
	public static function loadAnimateAtlas(spr:flxanimate.FlxAnimate, folderOrImg:String):flxanimate.FlxAnimate
        {
	try {
		// This will look for Animation.json + spritemap1.json automatically
		spr.loadAtlas(folderOrImg);
	}
	catch (e:Dynamic) {
		FlxG.log.error('Failed to load AnimateAtlas from "$folderOrImg": $e');
	}
	return spr;
}
#end
